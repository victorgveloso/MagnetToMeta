...
const { filesAndSizeFromTorrentStream } = require('./files');
const { updateCurrentSeeders } = require('../seeders');
...
async function getStream(search, episode) {
    ...
    const stream = Promise.all(data.links.map(async(el) => {
      ...
      const seeders = await updateCurrentSeeders(torrentsInput)
      const files = await filesAndSizeFromTorrentStream(torrentsInput)
      const streams = files.files.map(el => {
          ...
          const data = {
            ...
            infoHash: torrentsInput.infoHash
          }
          return data
      })
