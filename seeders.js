...
async function getAllTorrentsTrackers(torrents) {
  return Promise.all(torrents.map(torrent => getTorrentTrackers(torrent)
...
function groupByTrackers(torrentsTrackers) {
...
function scrapeTrackersWithTimeout(mapTrackerInfoHash, callback) {
...
async function getSeedersPerTorrent(torrentsInput) {
  return new Promise(async (resolve) => {
    ...
    const torrentsTrackers = await getAllTorrentsTrackers(torrents);
    const perTrackerInfoHashes = groupByTrackers(torrentsTrackers);
    ...
    scrapeTrackersWithTimeout(perTrackerInfoHashes, callback);
  });
}

async function updateCurrentSeeders(torrents) {
  let perTorrentResults = getSeedersPerTorrent(torrents);
